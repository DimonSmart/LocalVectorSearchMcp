    ...
    IWorkspaceNavigationService navigation)
{
    ...
    [Description("Atomically edits Markdown using hashed semantic anchors, commits the source file, and schedules index reconciliation. Public concrete pointers use logical~selfHash~subtreeHash with 16-character lowercase hashes. replace_element (and deprecated replace), insert_before, insert_after, and delete validate only selfHash, so an unchanged element can survive independent descendant edits. replace_section replaces a heading and all source in its section through the next heading with level <= the target level or end of document; it requires canonical v2 input and additionally validates subtreeHash so stale body or raw-content edits cannot be overwritten. Relocation uses only exact element kind plus selfHash. Legacy logical~selfHash input remains accepted for Self operations but is rejected for replace_section with a reread instruction. The document pointer remains unhashed and supports insert_before and insert_after at document boundaries. IndexSynchronized reports whether derived-index synchronization has already been confirmed.")]
